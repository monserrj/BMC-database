...
def protein_addition(session, protseq, NCBIid, uniprot, struct, dnaseq):
    ...
    # Try to commit our changes (don't commit 2024-11-26_LP)
...
if __name__ == "__main__":
    ...
    for idx, (
        ...
        try:  # attempt to add to db, if we raise an error, we roll back
            ...
            # else:
            #     print(f"{protein.taxonomies[0].__dict__=}")

            # No errors to here, commit session
            session.commit()
